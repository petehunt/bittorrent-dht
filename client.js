...
function idPrefixMatches(a, b) {
  return a[0] === b[0] && a[1] === b[1] && (a[2] & 0xf8) === (b[2] & 0xf8);
}

const DHT_SEC_IPV4_MASK = Buffer.from("030f3fff", "hex");
const DHT_SEC_IPV6_MASK = Buffer.from("0103070f1f3f7fff", "hex");

function calculateIdPrefix(addr, r) {
  var buf = Buffer.from(addr.split("."));
  var mask = buf.length === 4 ? DHT_SEC_IPV4_MASK : DHT_SEC_IPV6_MASK;
  var n = Math.min(buf.length, mask.length);
  var arg = Buffer.alloc(n);
...
function isValidNodeId(id, addr) {
  ...
  const isValid = idPrefixMatches(prefix, id);
  return isValid;
}
...
class DHT extends EventEmitter {
  ...
  put(opts, cb) {
    ...
    if (
      isMutable &&
      typeof opts.sign !== "function" &&
      !Buffer.isBuffer(opts.sig)
    ) {
      throw new Error(
        "opts.sign function or options.sig signature is required for mutable put"
      );
    }
  ...
  lookup(infoHash, cb) {
    ...
    function onreply(message, node) {
      ...
      if (isValidNodeId(message.r.id, node.address)) {
  ...
  _closest(target, message, onmessage, cb) {
    ...
    function onreply(message, node) {
      ...
      if (
        ...
        const token = isValidNodeId(message.r.id, node.host || node.address)
          ? message.r.token
          : null;
        table.add({
          id: message.r.id,
    ...
    if (!id) {
      ...
      if (!ip) {
        ip = await new Promise((resolve, reject) => {
          ...
          rpc.closest(
            rpc.id,
            { q: "get_peers", a: { info_hash: rpc.id } },
            onreply,
            done
          );
          ...
          function onreply(message, node) {
            if (message.ip) {
              const ip = parseIp(message.ip, 0);
              seenIps[ip] = seenIps[ip] || 0;
              seenIps[ip]++;
            }
          }
          ...
          function done() {
            rpc.destroy(() => {
              let ips = Object.keys(seenIps);
              ips.sort((a, b) => seenIps[b] - seenIps[a]);
              if (ips.length === 0) {
                reject(
                  "could not determine external ip from bittorrent bootstrap"
                );
              } else {
                resolve(ips[0]);
              }
